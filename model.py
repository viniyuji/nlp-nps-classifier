...
def CleanData(data):
    ...
    data.cluster = data.cluster.replace(to_replace = "-", value = "Nada")
    
    cluster_to_int = pd.DataFrame({'cluster': data.cluster.unique(), 'value': range(len(data.cluster.unique()))})

    for i in range(len(cluster_to_int)):
        data.cluster = data.cluster.replace(to_replace = cluster_to_int.cluster[i], value = cluster_to_int.value[i])

    x = data.melhorar.to_numpy(dtype='str')
    y = data.cluster.to_numpy(dtype='int16')
    return x,y
